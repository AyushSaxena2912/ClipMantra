...
const JobDetailPage = ({ job: initialJob, onBack, toast, onRefresh }) => {
  ...
  useEffect(() => {
    if (job.highlights_path) {
      ...
      fetch(`http://localhost:8000/storage/${path}`)
        .then((r) => (r.ok ? r.json() : []))
  ...
  const startStream = useCallback(() => {
    ...
    const es = new EventSource(`${API_BASE}/jobs/${job.id}/stream?token=${getToken()}`);
    esRef.current = es;
  ...
  useEffect(() => {
    const active = ["queued","downloading","transcribing","rendering","processing"].includes(job.status);
    if (active) startStream();
    return () => { if (esRef.current) esRef.current.close(); };
  }, []);

  const currentStep = liveStatus === "completed"
    ? STEPS.length - 1
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        {["queued","downloading","transcribing","rendering","processing"].includes(liveStatus) && !streaming && (
                ...
                <div key={step} style={{ display: "flex", alignItems: "center", flex: i < STEPS.length - 1 ? 1 : "none" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                    <div style={{
                      ...
                      height: 28,
                      borderRadius: "50%",
                      background: done ? "#00e59922" : active ? "#f0a50022" : "#111",
                  ...
                  {i < STEPS.length - 1 && (
                    <div style={{
                      flex: 1,
                      height: 2,
                      background: done ? "#00e59944" : "#1a1a1a",
                      margin: "0 4px",
                      marginBottom: 22
                    }} />
                  )}
          ...
          <div style={{
            ...
            {clips.map((clipPath, i) => {
              ...
              const videoUrl = clipPath.startsWith("http")
                ? clipPath
                : `http://localhost:8000/storage/${clipPath.replace(/^storage\//, "")}`;
